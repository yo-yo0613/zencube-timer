...
const AppContent = () => {
  ...
  // Keep navigation active checking standard route paths
  const getIsActive = (targetPath) => {
    if (targetPath === '/') {
      return path === '/'
    }
  ...
  return (
    <div className="min-h-screen bg-brand-light dark:bg-brand-dark text-brand-dark dark:text-brand-light pb-16 md:pb-0">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="flex gap-1.5 text-xs font-bold">
            {[
              ...
              const active = getIsActive(item.to)
              return (
                <Link
                  key={item.to}
                  to={item.to}
      ...
      {/* 
        Main Content Area: State-preserving display toggling.
        Renders all page components concurrently and controls visibility 
        using block/hidden styles to prevent unmounting and preserve local states.
      */}
      <main className="transition-all duration-300">
        <div className={path === '/' ? 'block' : 'hidden'}><Home /></div>
        <div className={path === '/formulas' ? 'block' : 'hidden'}><Formulas /></div>
        <div className={path === '/csp-trainer' ? 'block' : 'hidden'}><CspTrainer /></div>
        <div className={path === '/blindfold' ? 'block' : 'hidden'}><Blindfold /></div>
        <div className={path === '/history' ? 'block' : 'hidden'}><History /></div>
        <div className={path === '/auth' ? 'block' : 'hidden'}><Auth /></div>
      </main>
